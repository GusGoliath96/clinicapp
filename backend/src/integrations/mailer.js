...
function moldura(titulo, corpo) {
  ...
  <h3 style="margin:0 0 12px;font-size:16px;font-weight:600">${titulo}</h3>
  ${corpo}
...
export function enviarCodigo2fa(email, codigo) {
  return post({
    from: env.mail.from,
    to: email,
    ...
    html: moldura(
      ...
      `<p style="margin:0 0 16px;font-size:14px">Use o código abaixo para entrar. Ele vale por <strong>10 minutos</strong>.</p>
       <p style="margin:0;font-size:32px;font-weight:700;letter-spacing:6px;color:#1e40af">${codigo}</p>`,
    ),
  });
}

export function enviarLinkReset(email, url) {
  return post({
    from: env.mail.from,
    to: email,
